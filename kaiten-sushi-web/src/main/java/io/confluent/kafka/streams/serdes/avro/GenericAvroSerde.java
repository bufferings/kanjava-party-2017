...
/**
 * A schema-registry aware serde (serializer/deserializer) for Apache Kafka's
 * Streams API that can be used for reading and writing data in "generic Avro"
 * format. This serde's "specific Avro" counterpart is
 * {@link SpecificAvroSerde}.
 *
 * <p>
 ...
 *
 * <p>
 * Example for configuring this serde as a Kafka Streams application's default
 * serde for both record keys and record values:
 * </p>
 *
 * <p>
 * 
 ...
 *
 * <p>
 * Example for explicitly overriding the application's default serdes (whatever
 * they were configured to) so that only specific operations such as
 * {@code KStream#to()} use this serde:
 * </p>
 *
 * <p>
 * 
...
public class GenericAvroSerde implements Serde<GenericRecord> {
  ...
  GenericAvroSerde(final SchemaRegistryClient client) {
    ...
    inner = Serdes.serdeFrom(new GenericAvroSerializer(client), new GenericAvroDeserializer(client));
  }
