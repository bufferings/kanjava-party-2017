...
@Component
public class InventoryReservationRequestedEventSchema {

  private static final String SCHEMA_RESOURCE = "classpath:avro/InventoryReservationRequestedEvent.avsc";

  private final Schema schema;

  @Autowired
  public InventoryReservationRequestedEventSchema(@Value(SCHEMA_RESOURCE) Resource schemaResource) throws IOException {
    try (InputStream is = schemaResource.getInputStream()) {
      schema = new Schema.Parser().parse(is);
    }
  }

  public InventoryReservationRequestedEvent eventFrom(GenericRecord record) {
  ...
  public GenericRecord recordFrom(InventoryReservationRequestedEvent event) {
